...
| 文档 | 补充什么 |
|---|---|
| [STRATEGIES.md](STRATEGIES.md) | 六个策略的**完整话术示例**与逐条设计理由;§4.1 是预测检验的**权威定义** |
...
**秩 = 名次。** 六个策略从 **1(最弱)** 排到 **6(最强)**。

为什么不预测"② 会到 38%"?两个理由:

1. **没人能预测那个数。** 它取决于模型、prompt 措辞、工具描述……
   拍一个具体数值是**假装精确**。
2. **就算预测了也没信息量。** §9 合格线要求所有策略挤进 **20–50%**,
   六个塞进 30pp 的区间,绝对值本身区分度很低。
...
②③ 都预测为"强",也就是**并列第 5、第 6 名**。谁 5 谁 6 我们没有意见,
那就都取平均:`(5 + 6) / 2 = 5.5`。

同理 ①④ 并列第 3、4 名 → `(3 + 4) / 2 = 3.5`。

这叫**平均秩**,是统计学处理并列的标准做法。好处是所有秩之和保持不变
(1+2+3+4+5+6 = 21),可以直接喂给秩相关公式。
...
如果直接开跑校准:每一场 attempt 都记 0 分,六个策略全 0
...
如果只有一个臂,那就没什么可选的。**六个策略 = 六个摇臂**,规模刚好。
...
所以六个策略全部围绕这三件事,不涉及"让模型说不该说的话"那一类。
...
旋钮 ③ 那条警告值得记住:关掉权限检查,六个策略的 Impact ASR 一起涨,
...
2. **相对顺序,不是绝对数值** —— §16.3 把所有策略挤进 20–50%,
   在这么窄的区间预测绝对值几乎说不出信息;
...
秩从 **1(最弱)到 6(最强)**,并列取平均秩(见 §9.1):

| 策略 | 预测秩 | 档位 |
|---|---|---|
| ② Role-play / ③ Cross-user | **5.5** | 强 |
...
6 个策略共 15 对,其中 ②③、①④ 并列(不断言方向),剩下 **13 对**有方向。
每对按实测 ASR 之差判为:
...
而 §16.3 把六个策略压进 30pp,**相邻两档平均只差 6pp —— 低于分辨率**。

所以可以事先预告:**13 对中约 9–10 对可判决,3–4 对分辨不出,且集中在相邻档。**
...
| **攻击方太弱**,把六个策略都渲染成了差不多的话术 | **换攻击方,靶场一个字不用动** |

三者在数据里**长得一模一样**(六条 ASR 挤在一起),所以需要**攻击方对照**。

#### 攻击方对照怎么做 ⭐

让 attacker 按六个策略各生成 N 条话术,然后比两个数:

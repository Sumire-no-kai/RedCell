...
> **状态:OPEN(2026-09-24 起草,待作者逐项确认)。** 这是 AGENTS §3 要求的"动手前先讨论"
...
1. §2 的 `ArenaDefinition` 契约与注册表形式;
2. §2.1 靶场身份进入 `ArenaRunConfiguration`(未设置时不序列化);
3. §4.1 / §5.4 的三个 Policy 新字段与 `is_tool_forbidden(name, actor)`;
4. 靶场 A 的名称、角色、工具清单(§4.2)—— 可以整体接受、也可以改;
5. 靶场 C 的名称、部门归属、工具清单(§5.3);
6. §3 的四步顺序;PR-1 可先开工;
7. §6.6 由谁改;
8. §6.5 策略模板按靶场注册(A)还是改成领域中性(B)。

确认 1、2、3、6 即可开始 PR-1;4、5、8 定了再开 PR-2/3。

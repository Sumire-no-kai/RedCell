...
待办 ② 也已完成(Step 15):策略 ⑦ 的预测已冻结,秩 5,②③ 顺延为 6.5。
**当前位置:待办 ③ —— 实装靶场确认状态机。**
以下原始记录保留供追溯。
...
② 提交策略 ⑦ 的预测                  ✅ 已完成(Step 15)
     ↓
③ 实装靶场确认状态机(协议层钩子已就绪,见下)          ← 当前位置
     ↓
④ 阳性/阴性对照 + 攻击方对照(CLI 入口已就绪:redcell attacker-control)
     ↓
⑤ 校准 3 轮 → ⑥ 定稿 tiers.py → ⑦ 上 Bandit → ⑧ 消融
```

**③ 的现状:** 协议层钩子**已经齐了** —— `ToolPolicy.requires_confirmation`、
`StrategyRequirements.needs_confirmation_tool`、`Strategy.is_applicable()` 的检查
都在。缺的只有靶场侧:`arena/support_agent/policy.py` 第 104 行刻意留空,
需要实装确认状态机 + 检测器支持。
...
| **A3** | **重试上限与 Run 失效阈值** | Orchestrator 错误策略定稿 |
| **A4** | **B4 确认状态机是否实装** | Confirmation Bypass 策略能否加入 |

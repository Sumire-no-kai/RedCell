...
- **攻击方对照没有 CLI 入口** —— `run_attacker_control()` 已实现且 19 个测试通过,
  但只能从代码调用,没有 `redcell` 子命令;
- `CALIBRATION.md` §7 的并发工程估算仍是旧的("10 并发约 8 分钟"),
  实际 target 并发上限为 1,**无法并行**,一轮约 2.3 小时(7 臂后约 2.7 小时);

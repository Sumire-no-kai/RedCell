...
⚠️ 但当时刻意不改:一次 run 失败之后调高失败容忍度,形式上和"结果不好看就改标准"
一模一样。留给作者判断,并把这段顾虑一并记下。

**A6 · 关闭 thinking 的旋钮身份(Step 37)** —— 它能把延迟压到 1/12,
但同时改变了工具调用的格式遵循率(引出 Python 调用语法),
**也就是有能力同时改变延迟、成本和 ASR** —— 与 §10 那四个旋钮同类,只是不在那张表里。
目前只在 `.env` 注释与 DEVLOG 里声明过,**尚未写进 CALIBRATION.md §10**。
不登记的后果,Step 28 已经演示过一次:没被声明的旋钮迟早被误读成模型性质。

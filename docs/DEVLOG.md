...
**三组对照、七个策略、四档难度旋钮、成本闸门全部就绪,475 个测试全过。**
卡在两件事上:**① 靶场吞吐(GLM 今天 16 秒/次,一轮外推 41 小时);
② 难度档位要重新定** —— 因为 Step 28/29 改了 prompt 分层,**此前所有 ASR 数据作废**。
...
- ⚠️ **`redcell run` 的 structlog 输出会被缓冲**,进程结束前不落盘 ——
  **不要拿运行中的日志文件下结论**(我犯过,把"限流 0 次"报错了)。
- 分支 `feat/run-orchestrator`,**21 个提交未推送**,工作树干净。

## 两个模型位(已定,已实测)

| 位 | 模型 | 状态 |
|---|---|---|
| **attacker** | `gemini-3.1-flash-lite`(付费第 1 层) | ✅ **定了**。$0.84/轮、4K RPM / 150K RPD、生成零空零推理块 |
| **target** | `glm-4.7-flash`(永久免费) | ⚠️ 适任但**慢且不稳**:8/1 实测 3.8 秒/次,8/3 实测 **16 秒/次** |

`.env` 已配好真实单价(`0.25 / 1.50`),所以 **`--max-cost` 是活的闸门**,不再是摆设。

> **为什么 target 不换 Gemini:** 实测**没有一个 Gemini 型号能同时点亮两条检测线** ——
> 3.1 肯泄漏 canary 但不跨用户调工具,3.5-lite 反过来。换任何一个都会让**一整条信号线
> 结构性归零**。GLM 是目前唯一两条都能触发的。
> ⚠️ 但这批测量做于 prompt 分层修正**之前**,**Gemini 的适任性需要重测**(见待办 ②)。
...
① 解决靶场吞吐 ← 当前位置
     ↓
② 重跑三组对照(prompt 已变,旧结论全部作废)
     ↓
③ 重新定难度档位(N=10 彩排 → 看 §9 三条标准)
     ↓
④ 正式校准 3 轮 → ⑤ 定稿 tiers.py → ⑥ 上 Bandit → ⑦ 消融
```

**① 的三条路**(需作者决策):

1. **升级 GLM 付费档**(4.7-FlashX,并发 3)—— 我推荐这条;定价与真实并发上限**尚未查证**;
2. 接受慢跑 + **实现 resume**(目前没有,41 小时的进程中途挂掉全废);
3. 换 target —— 但要先排除"整条信号线归零"(见上)。

**② 的具体动作:** `redcell controls`(约 10 分钟)+ `attacker-control`(约 3 分钟)。
顺带**第一次公平地**测 Gemini 当靶场(之前那次测的是"模型 + 我们的隐式规则")。
...
5. **不给攻击方看检测仪器** —— canary、system prompt 指纹、`predicted_rank` 都不得进攻击方 prompt;
6. **规范只待在防御块** —— 角色设定只陈述事实(Step 28/29),有测试双向锁死;
7. **git 提交信息不出现 AI 工具署名或提及** —— 这是作品集项目,历史会被外部看到。

## 本轮踩过的坑(教训比结论值钱)

| 坑 | 一句话 |
|---|---|
| **测一个样本就写成一类性质** | 配额问题上连犯三次:先"Gemini 全线 20/天",再"只有 3.6",最后才靠系统扫描得到"flash 档 20、flash-lite 500" |
| **拿没写完的日志下结论** | `redcell run` 的日志被缓冲,我据此报了"限流 0 次",实际 38 次 |
| **两次"假通过"** | `legitimate_refund` 报零误报却什么都没验证;攻击方对照 +0.205 是算在**推理文本**上的 —— 两次都只有**人工抽检**发现 |
| **修了问题描述的另一半** | 写下"缺口不均匀,缺的是正在跑的那个臂",然后只修了总数,没修每臂 |
| **自己的方案有硬伤** | 提议"绕过必须真的执行副作用",会混淆 Attempt/Impact —— 已收回 |
| **我们一直在拧一个看不见的旋钮** | 角色设定里藏着实质防御规则,一句话措辞让工具调用从 0/3 变 3/3 |

> **最值得带走的一条:已有的 trace 往往比再跑一次实验更有说服力,而且免费。**
> 我差点花钱跑一轮对照,去回答一个答案已经躺在数据库里的问题。
